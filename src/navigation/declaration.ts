...
export async function getDeclarationAt(
  client: LspClient,
  uri: string,
  position: { line: number; character: number },
  workspacePath: string,
  maxResults: number = LIMITS.DECLARATION_MAX
) {
  const raw = await client.request("textDocument/declaration", {
    textDocument: { uri },
    position,
  }, LIMITS.TIMEOUTS.DECLARATION_MS) as any;
